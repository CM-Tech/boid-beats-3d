...
function Microphone(_fft) {
  ...
  function init() {
    try {
      ...
      console.error(e);
      alert(
        "Web Audio API could not be called, check that the url starts with https:// and not http://",
      );
    }
  }
  this.init = init;
  ...
  function processSound(stream, context) {
    ...
    // Use requestAnimationFrame for polling instead of deprecated ScriptProcessor
    function updateSpectrum() {
      ...
      self.animationId = requestAnimationFrame(updateSpectrum);
    }
    updateSpectrum();
  }
}
...
function initComputeRenderer() {
  ...
  colorVariable = gpuCompute.addVariable(
    "textureColor",
    document.getElementById("fragmentShaderColor").textContent,
    dtColor,
  );
...
function render() {
  ...
  birdUniforms["textureColor"].value =
